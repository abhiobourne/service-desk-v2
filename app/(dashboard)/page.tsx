...
export default function NextGenDashboard() {
  ...
  const [deploymentMarkers, setDeploymentMarkers] = useState<MapMarker[]>(DEPLOYMENT_MARKERS);
  ...
  const activeGlbUrl = React.useMemo(() => {
    ...
    return glbFile ? `http://localhost:7000${glbFile.url}` : null;
  }, [troubleshootingTree, selectedTreeNodeKey]);
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        setDeploymentMarkers(markerList.length ? markerList : DEPLOYMENT_MARKERS);
    ...
    fetchTroubleshootingByProduct(twinProductId).then(res => {
      if (res.success && res.data.length > 0) {
        ...
        for (const node of res.data) {
          ...
          if (glb) { setTwinGlbUrl(`http://localhost:7000${glb.url}`); break; }
        }
      }
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 overflow-hidden bg-[#07090e]">
        ...
        {/* HEADER BRAND / STAT BAR */}
        <header className="h-16 border-b border-white/5 px-8 flex items-center justify-between shrink-0 bg-[#090b10]/60 backdrop-blur">
          <div className="flex items-center gap-6">
            <div className="text-xs font-mono tracking-widest uppercase text-white/50 flex items-center gap-2">
            ...
            <div className="hidden md:flex items-center gap-4">
              ...
              <span className="text-[10px] font-mono text-[#a3e635] uppercase">BIOMETRIC AUTHORIZED</span>
            </div>
            ...
            {isClient && (
              <div className="hidden lg:flex items-center gap-4">
                ...
                <span className="text-[10px] font-mono text-cyan-400 uppercase">
                  CLIENT: {user?.client?.name || "Portal Access"}
                </span>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              onClick={() => alert("Telemetry data logs generated for fleet audit.")}
              className="px-3 py-1.5 border border-white/10 hover:border-white/20 text-white/80 text-[11px] font-mono rounded flex items-center gap-1 transition uppercase"
            >
            ...
            <div className="relative">
              <button className="p-2 text-white/60 hover:text-white rounded bg-white/5 border border-white/5 hover:border-white/10 transition">
            ...
            {user && (
              <div className="flex items-center gap-3 pl-2 border-l border-white/10">
                <div className="flex flex-col text-right">
                  ...
                  <span className="text-[9px] font-mono text-[#06b6d4] uppercase tracking-wider">
                    {roleName}
                  </span>
                </div>
                <button
                  ...
                  className="h-8 w-8 rounded-full border border-[#06b6d4]/40 hover:border-red-500/50 bg-[#121620] overflow-hidden flex items-center justify-center font-mono text-xs font-bold text-[#06b6d4] hover:text-red-400 transition"
                >
        ...
        <div className="flex-1 overflow-y-auto p-8">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-[#0c0e16] border border-white/5 p-6 rounded-lg flex flex-col">
                  ...
                  <div className="flex-1 min-h-[300px] rounded overflow-hidden">
                    <WorldMap markers={deploymentMarkers} />
                  </div>
                </div>
          ...
          {activeTab === "twin" && (() => {
            ...
            const TELEMETRY = [
              { label: "Nitrogen", value: "98.2", unit: "%", color: "text-cyan-400", warn: false },
              { label: "Core Temp", value: "4.2", unit: "K", color: "text-emerald-400", warn: false },
              { label: "Pressure", value: "42.5", unit: "psi", color: "text-amber-400", warn: true },
              { label: "Sys Load", value: "74", unit: "%", color: "text-white", warn: false },
            ];
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <button
                      ...
                      className={`px-3.5 py-2 text-xs font-mono uppercase rounded font-bold transition flex items-center gap-1.5 ${
                        diagnosticRunning
                ...
                <div className="flex flex-wrap gap-2">
                  {INV_CATEGORIES.map(cat => (
                    <button
                      ...
                      className={`px-4 py-1.5 rounded-full text-xs font-mono font-semibold border transition ${
                        invCategory === cat
                        ...
                        <div key={item.id} className="bg-[#0c0e16] border border-white/5 rounded-xl p-5 flex flex-col gap-3 hover:border-white/10 transition group">
                          ...
                          <div className="flex gap-2 mt-auto">
                            ...
                            <button
                              ...
                              className={`flex-1 py-2 text-[10px] font-mono uppercase rounded-lg font-bold transition border ${
                                item.stock.label === "Out of Stock"
                      ...
                      troubleshootingTree.map((node) => {
                        ...
                        return (
                          <div 
                            ...
                            className={`group flex items-center justify-between py-1.5 px-2 rounded cursor-pointer transition text-xs font-mono ${
                              isSelected 
                ...
                <div className="lg:col-span-2 space-y-4">
                  ...
                  <div className="space-y-4 overflow-y-auto max-h-[550px] pr-2">
                    {tickets.map((t) => (
                      <div 
                        key={t.id} 
                        className={`bg-[#0c0e16] border p-6 rounded-lg space-y-4 hover:border-blue-500/30 transition duration-300 ${
                          t.priority === "CRITICAL" ? "border-rose-500/20 bg-rose-950/[0.03]" : "border-white/5"
                        }`}
                      >
