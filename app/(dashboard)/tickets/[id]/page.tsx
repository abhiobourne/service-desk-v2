...
function getFileTypeInfo(url: string) {
  ...
  if (ext === "txt")              return { Icon: FileType, label: "TXT",  bg: "bg-slate-100 dark:bg-white/5",       ic: "text-slate-400 dark:text-white/30",   badge: "bg-slate-100 dark:bg-white/5 border-slate-200 dark:border-white/10 text-slate-500 dark:text-white/30" };
  return { Icon: FileIcon, label: ext.toUpperCase() || "FILE", bg: "bg-slate-100 dark:bg-white/5", ic: "text-slate-400 dark:text-white/30", badge: "bg-slate-100 dark:bg-white/5 border-slate-200 dark:border-white/10 text-slate-500 dark:text-white/30" };
}
    ...
    <div className="bg-white dark:bg-[#090b10] border border-slate-200 dark:border-white/5 rounded-xl overflow-hidden">
      <div className="p-5">
        <div className="flex items-center justify-between gap-4 mb-4">
          <div>
            <h3 className="text-sm font-mono font-semibold text-slate-700 dark:text-white/70">Faulty Parts</h3>
            <span className="text-[10px] font-mono text-slate-400 dark:text-white/25">{parts.length} parts</span>
          </div>
          <div className="relative w-56">
            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3 h-3 text-slate-300 dark:text-white/20 pointer-events-none" />
            <input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="Search parts…"
              className="w-full bg-slate-50 dark:bg-white/3 border border-slate-200 dark:border-white/8 text-slate-700 dark:text-white text-[10px] font-mono pl-7 pr-3 py-2 rounded-lg focus:outline-none focus:border-blue-400 dark:focus:border-violet-500/40 placeholder:text-slate-400 dark:placeholder:text-white/20" />
          </div>
        </div>
        {parts.length === 0 ? (
          <div className="flex items-center justify-center h-16 text-xs font-mono text-slate-400 dark:text-white/25">No faulty parts reported.</div>
        ) : (
          <>
              ...
              <thead>
                <tr className="border-b border-slate-100 dark:border-white/5">
                  {["Part Name", "Type", "Version", "View"].map((h) => (
                    <th key={h} className="px-3 py-2.5 text-left text-[9px] font-mono font-bold text-slate-400 dark:text-white/25 uppercase tracking-widest">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {slice.length === 0 ? (
                  <tr><td colSpan={4} className="px-3 py-8 text-center text-[10px] font-mono text-slate-400 dark:text-white/25">No results for &ldquo;{search}&rdquo;</td></tr>
                ) : slice.map((p, i) => (
                  <tr key={i} className="border-b border-slate-50 dark:border-white/3 last:border-0 hover:bg-slate-50 dark:hover:bg-white/2">
                    <td className="px-3 py-2.5 text-xs font-mono font-medium text-slate-700 dark:text-white/70">{p.part_name}</td>
                    <td className="px-3 py-2.5 text-xs font-mono text-slate-500 dark:text-white/40">{p.part_type}</td>
                    <td className="px-3 py-2.5"><code className="text-[10px] font-mono text-slate-400 dark:text-white/30">{p.part_version ?? "—"}</code></td>
                    <td className="px-3 py-2.5">
                      {p.troubleshooting_url
                        ? <a
                            ...
                            className="text-[10px] font-mono text-blue-500 dark:text-violet-400 hover:text-blue-700 dark:hover:text-violet-300"
                          >View →</a>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between mt-3">
                <span className="text-[9px] font-mono text-slate-400 dark:text-white/20">Page {page} of {totalPages}</span>
                <div className="flex gap-1">
                  <button disabled={page <= 1} onClick={() => setPage((p) => p - 1)} className="w-6 h-6 flex items-center justify-center rounded border border-slate-200 dark:border-white/8 text-slate-400 dark:text-white/25 hover:bg-slate-100 dark:hover:bg-white/5 disabled:opacity-30 transition"><ChevronLeft className="w-3 h-3" /></button>
                  <button disabled={page >= totalPages} onClick={() => setPage((p) => p + 1)} className="w-6 h-6 flex items-center justify-center rounded border border-slate-200 dark:border-white/8 text-slate-400 dark:text-white/25 hover:bg-slate-100 dark:hover:bg-white/5 disabled:opacity-30 transition"><ChevronRight className="w-3 h-3" /></button>
                </div>
              </div>
            )}
          </>
...
export default function TicketDetailPage() {
  ...
  if (authLoading || loading) {
    return (
      <div className="flex-1 flex items-center justify-center bg-slate-50 dark:bg-[#06070a]">
        <Loader2 className="w-6 h-6 text-blue-500 dark:text-violet-400 animate-spin" />
      </div>
    );
  }
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-slate-50 dark:bg-[#06070a]">
        <div className="text-center">
          ...
          <button onClick={() => router.back()} className="mt-4 text-xs font-mono text-slate-400 dark:text-white/40 hover:text-slate-700 dark:hover:text-white/70 transition">Go back</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full overflow-hidden bg-slate-50 dark:bg-[#06070a] text-slate-900 dark:text-white">
        ...
        <div className="max-w-4xl mx-auto px-6 py-6 space-y-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-5">
            ...
            <div className="bg-white dark:bg-[#090b10] border border-slate-200 dark:border-white/5 rounded-xl p-5 space-y-4">
              <h3 className="text-sm font-mono font-semibold text-slate-600 dark:text-white/70">Ticket Information</h3>
              <div className="space-y-2">
                {[
                  ...
                  { icon: Package, label: "Product", value: ticket.items?.[0]?.product_name ?? (ticket as any).product_name ?? (ticket as any).line_items?.[0]?.product_name ?? ticket.product_id ?? "—" },
                ].map(({ icon: Icon, label, value, mono }) => (
                  <div key={label} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-white/3 rounded-lg">
                    <div className="flex items-center gap-2 text-slate-400 dark:text-white/35 text-xs font-mono">
                      <Icon className="w-3.5 h-3.5 shrink-0" />{label}
                    </div>
                    {mono
                      ? <code className="text-[11px] font-mono text-slate-600 dark:text-white/55 truncate max-w-[220px]">{value}</code>
                      : <span className="text-xs font-mono font-medium text-slate-700 dark:text-white/65 truncate max-w-[220px]">{value}</span>}
                  </div>
                ))}
              </div>
              ...
              {(ticket.reason || ticket.description) && (
                <div className="pt-4 border-t border-slate-100 dark:border-white/5 space-y-3">
                  {ticket.reason && (
                    <div>
                      <p className="text-[9px] font-bold text-slate-400 dark:text-white/25 uppercase tracking-widest mb-1.5">Reason</p>
                      <div className="text-xs font-mono text-slate-600 dark:text-white/50 leading-relaxed" dangerouslySetInnerHTML={{ __html: ticket.reason }} />
                    </div>
                  )}
                  {ticket.description && (
                    <div>
                      <p className="text-[9px] font-bold text-slate-400 dark:text-white/25 uppercase tracking-widest mb-1.5">Description</p>
                      <div className="text-xs font-mono text-slate-600 dark:text-white/50 leading-relaxed" dangerouslySetInnerHTML={{ __html: ticket.description }} />
                    </div>
                  )}
                </div>
              )}
              {!ticket.reason && !ticket.description && (
                <p className="text-xs font-mono text-slate-300 dark:text-white/20 pt-2 border-t border-slate-100 dark:border-white/5">No description provided.</p>
              )}
            </div>

            {/* Right — Attachments */}
            <div className="bg-white dark:bg-[#090b10] border border-slate-200 dark:border-white/5 rounded-xl p-5 space-y-3">
              <h3 className="text-sm font-mono font-semibold text-slate-600 dark:text-white/70">Attachments</h3>
              {attachmentUrls.length > 0 ? (
                    ...
                    {attachmentUrls.map((url, i) => {
                      ...
                      return (
                        <div key={i} onClick={() => setPreviewUrl(url)}
                          className="relative group cursor-pointer h-[150px] rounded-lg border border-slate-200 dark:border-white/8 bg-slate-50 dark:bg-white/3 hover:border-blue-300 dark:hover:border-violet-500/30 transition overflow-hidden flex items-center justify-center">
                          {isImage ? (
                            ...
                            return (
                              <div className={`flex flex-col items-center justify-center gap-2 w-full h-full p-3 ${bg}`}>
                                ...
                                <span className="text-[9px] font-mono text-slate-400 dark:text-white/30 truncate w-full text-center px-1">{fileName}</span>
                              </div>
                            );
                          })()}
                ...
                <div className="flex flex-col items-center justify-center h-[300px] rounded-lg border border-dashed border-slate-200 dark:border-white/8 bg-slate-50 dark:bg-white/2 gap-3">
                  <div className="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-white/5 flex items-center justify-center">
                    <ImageIcon className="w-5 h-5 text-slate-300 dark:text-white/15" />
                  </div>
                  <p className="text-xs font-mono text-slate-400 dark:text-white/25">No attachments</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ background: "rgba(0,0,0,0.8)" }} onClick={(e) => { if (e.target === e.currentTarget) setPreviewUrl(null); }}>
            <div className="bg-white dark:bg-[#090b10] border border-slate-200 dark:border-white/8 rounded-2xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
              <div className="h-12 shrink-0 border-b border-slate-100 dark:border-white/5 flex items-center justify-between px-4">
                <span className="text-xs font-mono text-slate-500 dark:text-white/50 truncate flex-1">{previewUrl.split("/").pop()}</span>
                <div className="flex items-center gap-2">
                  <button onClick={() => downloadFile(previewUrl, previewUrl.split("/").pop() ?? "attachment")}
                    className="flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-mono text-slate-500 dark:text-white/40 border border-slate-200 dark:border-white/10 rounded hover:bg-slate-50 dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-white transition">
                    <Download className="w-3 h-3" />Download
                  </button>
                  <button onClick={() => setPreviewUrl(null)} className="text-slate-400 dark:text-white/30 hover:text-slate-700 dark:hover:text-white/70 transition p-1"><span className="text-lg">×</span></button>
                </div>
              </div>
      ...
      {/* Right: inline chat */}
      <div className="w-[360px] shrink-0 border-l border-slate-200 dark:border-white/5 overflow-hidden flex flex-col">
        <InlineTicketChat
          ticket={ticket}
